...
Dictionary<byte, int> bitCount = new Dictionary<byte, int>();
int input = int.Parse(File.ReadAllText(inputFile));
...
//void DrawGrid(Dictionary<(int x, int y), int> distances)
//{
...
IEnumerable<(int x, int y)> GetAdjacentPositions((int x, int y) pos)
{
    bool up = pos.y > 0;
    bool left = pos.x > 0;
