...
TreeNode.inputString = File.ReadAllText(inputFile);
...
void GetHashPassable(bool[] passable, string input)
{
...
static bool TranslateChar(char c)
{
    switch (c)
    {
        case 'b':
...
static void UpdatePassable(bool[] passable, int x, int y)
{
    if (x == 0)
    {
        passable[2] = false;
